...
s.close()
```
Before we execute, take note of the `\x90` instructions in the jump instruction. These two added instructions are called no operations that literally do nothing. They are just there as place holders, because the register needs four bytes- and we only had two. Carrying on...
