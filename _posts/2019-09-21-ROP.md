...
The reason why ROP is called Return Oriented Programming, is because each instructions is always followed by a `ret` instruction. Why is `ret`. Each `ASM + ret` instruction is known as a ROP gadget. Whenever these gadgets are loaded consecutively one after the other, this is known as a ROP chain.
...
Enter `ret`. A return instruction simply takes whatever is located in the stack pointer (on top of the stack) and loads it into the instruction pointer (what is currently being executed). Since the ROP chain is located on the stack, and a ROP chain is simply a bunch of memory addresses, teh `ret` instruction will simply return to the stack, pick up the next memory address (ROP gadget), and execute it. This will keep happening, until there are no more left! This makes life a bit easier.
