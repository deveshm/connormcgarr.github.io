...
The number that comes after `ret` refers to the number of items that should be cleared off of the stack. `C`, in decimal, is 12. 12 bytes would refer to 3 memory addresses in x86 (4 byte registers X 3 of them = 12). These types of returns are used to "clean up" items on the stack, by removing items.

In any case- just as `pop`, we will have to add some padding to compensate. As mentioned below, a `ret 0x000C` will remove 3 memory addresses off of the stack. First the return instruction takes the current stack pointer at the time of the `ret 0x000C` instruction (which would be the next ROP gadget in the chain) and loads it into EIP. That is why no padding is needed at this point. After that ROP gadget is executed, the `0x000C` portion of the return from the now previous ROP gadget kicks in, and takes 3 memory addresses off of the stack. This is the reason why padding for `ret NUM` instuctions are implemented in the NEXT ROP gadget instead of directly below, like `pop` padding.

This will be reflected and explained a bit better in the comments of the code for the updated POC that will include the `size` and `flNewProtect` parameters. 
