...
As you can see, there are a few `pop` instructions that need to be accounted for. We will add some padding to the updatedd POC, found below, to compensate:
...
Look at that! The memory address containing the `return` parameter (filled with `0x4c4c4c4c`) placeholder was successfully manipulated to point to the shellcode area!:
