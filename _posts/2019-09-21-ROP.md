...
..But ...But What About `jmp esp`?
---

There will not be a `jmp esp` instruction here. Remember, with DEP- this will kill the exploit. Instead, you'll need to find any memory address that contains a `ret` instruction. As outlined as below, this will directly take us back to the stack. This is normally called a _stack pivot_.
