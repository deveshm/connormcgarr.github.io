...
We can clearly see under the `kernel32.VirtualProtect` pointer, that has the `return` parameter located at `0x19FF9EC`.
...
We will repeat the following ROP gagdget multiple times:
...
Since `lpAddress` comes after `return` it will be located 4 bytes after `return` on the stack.
...
# (no pointers have been created yet. Just preperation)
...
Dissassembly window before execution: 
...
The current state of the stack. ESP contains the memory address `0x0189FA3C`, which points to `0x41414141`:
...
Right now , our shellcode is about 100 hex bytes, or about 256 bytes away, from the current `return` and `lpAddress` placeholders. Remember when earlierw e saved the old stack pointer into two registers: EAX and ECX? Recall also, that we have already manipulated the value of ECX to equal the value of the `return` parameter placeholder.
...
Let's recall the state of our registers up unil this point:

- __ECX__: location of `return` parameter placeholder
- __EDX__: location of `lpAddress` parameter placeholder
- __EAX__: location of shellcode (NOPS in front of shellcode)
...
The number that comes after `ret` refers to the number of bytes that should be removed from the stack. `C`, in decimal, is 12. 12 bytes would refer to three 4 byte values in x86 (Each 32 bit DWORD memory address contains 4 bytes. 4 bytes X 3 values = 12 total). These types of returns are used to "clean up" items on the stack, by removing items. Essentially, this just removes the next 3 memory addresses after the `ret` is executed.

In any case- just as `pop`, we will have to add some padding to compensate. As mentioned above, a `ret 0x000C` will remove 3 memory addresses off of the stack. First, the return instruction takes the current stack pointer at the time of the `ret 0x000C` instruction (which would be the next ROP gadget in the chain) and loads it into EIP. EIP then executes that address as normally. That is why no padding is needed at this point. After that EIP is executed, the `0x000C` portion of the return from the now previous ROP gadget kicks in, and takes 3 memory addresses removed off the stack. This is the reason why padding for `ret NUM` instuctions are implemented in the NEXT ROP gadget instead of directly below, like `pop` padding.
...
Remember, we now have to add padding for the last gadget's `ret 0x000C` instruction. This will take out 3 lines of addresses- so we insert three lines of padding:
...
Now, we repeate what we did earlier and create a pointer from the DWORD within EDX (the `size` parameter placeholder) to the value in EAX (the correct `size` parameter value). Reusing a previous ROP gadget:

```
0x77e942cb: mov dword [edx], eax ; pop esi ; pop ebp ; retn 0x000C ;  kernel32.dll
```

Again, that peksy `ret 0x000C` is present again. Make sure to keep a note of that. Also note the two `pop` instructions. Add padding to compensate there as well.
...
Again- we have to add padding for the last gadget's `ret 0x000C` instruction. Three addresses will be removed, so three lines of padding are neeeded:
...
Next we need the value of `0x40` in EAX. I could not find any viable pointers through any of the ROP gadgets I enumerated to add `0x40` directly. So instead, in typical ROP fashion, I had to make due with what I had.
...
rop2 += struct.pack ('<L', 0x41414141)   # padding to compensate for retn 0x000C in the lpAddress ROP gadget
rop2 += struct.pack ('<L', 0x6ff7e29a)   # 0x6ff7e29a: add eax, 0x00000100 ; pop ebp ; ret  ;  (1 found)
rop2 += struct.pack ('<L', 0x41414141)   # padding to compensate for pop ebp instruction in the above ROP chian
rop2 += struct.pack ('<L', 0x6ff7e29a)   # 0x6ff7e29a: add eax, 0x00000100 ; pop ebp ; ret  ;  (1 found)
...
The memory address of the `size` parameter now points to teh value of EAX, which is `0x300`!:
...
Then, EDX is incresed by four and the DWORD within EDX (the `flNewProtect` placeholder) it manipulated to point to the value of EAX- which is `0x40`! All of our parameters have successfully been added to the stack!:

<img src="{{ site.url }}{{ site.baseurl }}/images/DEP43.PNG" alt="">
...
As you can see, our `calc.exe` payload has been executed- and DEP has been defeated!!!!!:
