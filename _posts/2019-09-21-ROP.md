...
The reason why ROP is called Return Oriented Programming, is because each instructions is always followed by a `ret` instruction. Each `ASM + ret` instruction is known as a ROP gadget. Whenever these gadgets are loaded consecutively one after the other, this is known as a ROP chain.
