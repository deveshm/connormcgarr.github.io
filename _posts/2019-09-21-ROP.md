...
To find the addres of `VirtualProtect` on your machine, to to Immunity and double-clik on any instruction in the disassembly window and enter 
...
Now the POC can be updated, knowing the address:
...
# 5000 byte total crash
filler = "\x43" * (5000-len(command)-len(crash)-len(parameters))
s=socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect(("192.168.235.130", 9999))
s.send(command+crash+parameters+filler)
s.close()
```
...
It is pretty much guarenteed that there is no way we will find five ROP gadgets that EXACTLY equal the values we need.

Instead, what we will do is put the values needed into a register, and then change the placeholders we currently have, to point to the values in the registers.

An example would be, we could get the value for `lpAddress` into a register. Then, using ROP, we could make the current placeholder for `lpAddress` point to that register, where the intended value (real value) of `lpAddress` is.
