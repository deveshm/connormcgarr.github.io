...
The `recv()` call here again uses the same type of parameters as the previous call. Let's take a look at the declaration of the variable `local_238` in the above `recv()` function call, as it hasn't been referenced in this blog post yet.
...
- The specified length, which is represented by `local_20`. This variable was used in the check implemented above, which looks to see if the size is < 512 bytes. If the request has made it this far into eko2019.exe, this will always be 512 bytes or less.
- Zero, which represents what flags should be implemented (none in this case).
