...
We can potentially replace the `\x41` in front of our previous `mov rax, qword ptr [rcx]` instruction, and change it to create a variant of said instruction, `mov rax, qword ptr gs:[rcx]`! This would also mean that we would need to set RCX to 0x60 at the time of this instruction.
