...
After some research it seems as though the bytes `\x65\x48\x8b\x01` are used to create the instruction `mov rax, qword ptr gs:[rcx]`. This means we need to replace the `\x41` byte that cause our access violation with a `\x65` byte! Firstly, however, we need to identify where this byte is within our proof of concept.

Updating our proof of concept, we found that the byte we need to replace with `\x65` is at an offset of 512 into our 528 byte buffer. The bytes that control the value of RCX seem to come right after that! This was all found through trial and error.
...
This means now, that when we need to execute our code in the future- we can dynamically resolve our ROP gadgets via offsets- and ASLR will no longer be a problem! The only question remains- how the heck do we pivot back to the stack, to our big message buffer, with the very limited instruction set we have?
...
After some trial and error, I finally came to a pretty neat conclusion actually! We can load RCX with the address of our stack pivot ROP gadget. We can then replace the `\x41` byte from earlier (we changed this byte to `\x65` in the PEB portion of this exploit) with a `\x51` byte!

The `\x51` byte is the opcode that corresponds to the `push rcx` instruction! Push RCX will allow us to "push" our user controlled value of RCX onto the stack (Which is a stack pivot ROP gadget). Pushing an item on the stack, will actually load said item into RSP! This means that we can load our own ROP gadget into RSP, and then execute the `ret` instruction to leave the function- which will execute our ROP gadget! The first step for us, is to find a ROP gadget! We will use [rp++](https://github.com/0vercl0k/rp) to enumerate all ROP gadgets from eko2019.exe.
...
This gadget will raise the stack up in value, to load our user controlled values into RSP and subsequent bytes after RSP! Notice how each gadget does not show the full virtual address of the pointer. This is because of ASLR! If we look at the last 4 or so bytes, we can see that this is the offset from the base virtual address of eko2019.exe to said pointer. In this case, the ROP gadget we are going after is located at eko2019.exe + 0x158b.
...
exploit_3 += struct.pack('<Q', base_address+0x158b)

# Message needs to be 528 bytes total
exploit_3 += "\x41" * (544-len(exploit))
...
Now that we have been able to successfully pivot back to the stack, it is time to use ROP to execute some code. The challenge tells us that popping `calc.exe` is enough to prove code execution. We will be adhering to this notion! Let's ROP!
