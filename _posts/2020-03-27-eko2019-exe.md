...
Since we want to pop calc.exe, the first argument in this function should be the string `calc`, which should be null terminated. This should be stored in RCX. `lpCmdLine` (the argument we are fulfilling) is the name of the application we would like to execute.
