...
Number two, after the header is received by the program, the specific variable that contains the pointer to the buffer received by the previous `recv()` request (`local_28`) is compared to the string constant `0x393130326f6b45`, or `Eko2019` in an `if` statement.
...
If you can recall from the `WriteProcessMemory()` arguments, the buffer which `WriteProcessMemory()` will write to, is actually from the function `sub_140001170`. From the above assembly code, we can see how this function is utilized! It seems as though the unkown data type, `unk_14000E520` is what contains what is going to be written in the `WriteProcessMemory()` function call!

We already know that that `WriteProcessMemory()` will write the data from the function `sub_140001170` to the function `sub_140001000`. What is even more interesting, is that `WriteProcessMemory()` calls the location in which the buffer was written!

```nasm
call sub_140001000
```

Let's set a breakpoint on this location in memory in WinDbg, and start stepping through each instruction and dumping the contents of the registers in use, to see what we are working with here!
