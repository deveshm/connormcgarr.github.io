...
A few things stand out from the above image.

1. The `local_258` variable is initialized with the `recv` function. The `recv` function makes the function call with:
- An socket file descriptor, `param_1`, which is inherited from the function parameters
- A pointer where the buffer that was received will be written to (`local_28`)
...
The next line of code is a call to `printf()` which displays a message indicating the "header" has been received, with the length of `local_258` being displayed. Essentially, the length of the buffer received by eko2019.exe is stored in the variable `local_258`.

This is how the call looks, disassembled, within IDA.

<img src="{{ site.url }}{{ site.baseurl }}/images/BFS_7.png" alt="">

What this means, is that eko2019.exe seems to accept a header before the "message" portion of the client request. This header must be 0x10 hexadecimal bytes (16 decimal bytes) in length. This is the first "check" the application makes on our request, thus being the first "check" we must bypass.

2. After the header is received by the program, the specific variable that contains the header
