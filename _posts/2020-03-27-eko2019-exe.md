...
Here, we verify that the value from RCX above (with RAX appended to it), is called here. Essentially, when `sub_140001000` is called after the `WriteProcessMemory()` routine, it will land on whatever the value of the `sub_140001170` function returns, alone with some NOPS and a return.
...
After stepping through a few instructions, execution lands here, at ```nasm lea rcx,[eko2019+0xe520 (00007ff6`6641e520)]```.
