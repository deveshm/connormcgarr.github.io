...
The `recv()` call here again uses the same type of parameters as the previous call and reuses the variable `local_258`. Let's take a look at the declaration of the variable `local_238` in the above `recv()` function call, as it hasn't been referenced in this blog post yet.
...
Now that we have the ability to bypass the checks eko2019.exe makes on our buffer (which is comprised of the header and message), we can successfully interact with the server! The only question remains- where exactly does this buffer end up when it is received by the program? Will we even be able to located this buffer? Is this only a partial write? Let's take a look at the following snippet of code to find out.
...
The fifth parameter is located at RSP + 0x20. This is what the `__fastcall` calling convention defaults to after four parameters. Each parameter after 5 will be located 8 bytes after RSP + 0x20, at RSP + 0x28, RSP + 0x32, etc.
...
Stepping through, the value located at RSP+0x268 is moved into RCX- and then the location of where `WriteProcessMemory()` to is called! Stepping into that function, here is what we see, as outlined by the image below.

<img src="{{ site.url }}{{ site.baseurl }}/images/BFS_29.png" alt="">

Cool! This function receives the buffer from the `sub_140001170` (eko2019.exe+0x1170) function call. When those bytes are interpreted by the disassembler, you can see from the image above- this 8 byte QWORD is interpreted as an intruction that moves the value pointed to by RCX into RAX (with the NOPs we previously discovererd with IDA)! The function returns the value in RAX and that is the end of execution!
...
Awesome! As you can see, we were able to extract and view the contents of RAX remotely! This means that we can obtain some type of information leakage (although, it is not particuraly useful at the moment).

As reverse engineers, vulnearbility researchers, and exploit developers- we are taught never to accept things at face value! Although eko2019.exe tells us that we are not supposed to send a message longer than 512 bytes- let's see what happens when we send a value greater than 512! Adhering to the restriction about our data being in a multiple of 8, let's try sending 528 bytes to the server! Let's see what happens.
...
At this point, the value of RAX is about to be prepended to the following data again.

<img src="{{ site.url }}{{ site.baseurl }}/images/BFS_33.png" alt="">

Let's check out the contens of the RAX register, to see what is going to get appended here!

<img src="{{ site.url }}{{ site.baseurl }}/images/BFS_34.png" alt="">

Very interesting! It seems as though we actually control the byte in RAX! Now, if we step through the `WriteProcessMemory()` function call that will write this string and call it later on, we can see that this is why the program crashes.
...
This is what is very interesting! We actually control the contents of the RCX register. This essentially gives us an arbitrary read primtive due to the fact we can control what gets loaded into RCX, extract its contents into RAX, and return it remotely to the client! There are three things we need to take into consideration:

1. Where are the bytes in our buffer that are stored into RCX
2. What should we load into RCX?
3. Where is the byte that comes before the `mov rax, qword ptr [rcx]` instruction?
4. What should we change said byte to?

Let's address numbers two and three in the above list firstly.
...
We can potentially replace the `\x41` in front of our previous `mov rax, qword ptr [rcx]`, and change it into a `mov rax, qword ptr gs:[rcx]` instruction! This would also mean that we would need to set RCX to 0x60 at the time of this instruction.

Recall that we have the ability to control RCX at this time! This is perfect- we can use are ability to control RCX to load the value of 0x0000000000000060 into it- and access the GS segment register at this offset!

After some research it seems as though the bytes `\x65\x48\x8b\x01` are used to create the instruction `mov rax, qword ptr gs:[rcx]`. This means we need to identify where this byte is within our proof of concept.

Updating our proof of concept, we can see that this byte is at an offset of 512 into our 528 byte buffer and the bytes that control RCX come right after that. This was all found through trial and error.
...
As you can see from the image below, when we hit the move operation, we have got the correct instruction in place, and RAX now contains the value of PEB!
...
What is most devastating, that we can use to our advantage, is that the base address for the current process (eko2019.exe in this case) is located at an offset of PEB+0x10

<img src="{{ site.url }}{{ site.baseurl }}/images/BSF_40.png" alt="">

Essentially, all we have to do is use our ability to control RCX to load the value of PEB+0x10 into it. At that point, the application will extract the value into RAX to which PEB+0x10 points to- which is the actual base virtual address for eko2019.exe! This value will then be returned to the client! Note that this time we do not need to access the GS segment register. If you can recall, before we accessed the GS segment register, the program naturally executed a `mov rax, qword ptr[rcx]` instruction. To ensure this is the instruction executed this time, we will use our byte we control to implement a NOP- to slide into the intended instruction.

To do this, we will close our first connection to the client, and then make a second request! This update to our exploit development process is outlined in our updated proof of concept.
