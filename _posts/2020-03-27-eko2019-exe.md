...
The next line of code after the value of `local_258` is set, makes a call to `printf()` which displays a message indicating the "header" has been received, `local_258` being printed.
...
Now that we have our application reversed (as far as we need to reverse it), let's send eko2019.exe a buffer that adheres to all of the check bypasses while attached to WinDbg and see what happens!
