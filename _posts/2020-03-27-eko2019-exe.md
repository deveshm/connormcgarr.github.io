...
Taking a look at the data type of the `local_28`- we notice it is a `longlong`. This means that the variable should 8 bytes in totality. We notice, however, that `0x393130326f6b45` is only 7 bytes in length. This behavior is indicatory that the string of `Eko2019` should be null terminated. The null character will provide the last byte needed for our purposes.
