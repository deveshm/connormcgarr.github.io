...
In order to reach our goal, right now we only care about `pfn` which we can see from the `!pte` output. Let's understand the `pfn` output firstly, as this will help us understand the output of `!pte` and fetching a physical page associated with a virtual page.
...
The PFN, as we can see from the output in WinDbg, is `7bbc8`. This means this should be the page frame number for the PDPT, as we know a page frame number refers to the next paging structure in the hierarchy. We will now use `!vtop` to convert PDPT (PPE in WinDbg) to a physical address to verify that the PML4E entry is indexing the correct paging structure.
...
As we can see, the PDPT is located at a physical address of `7bbc8000`! This is perfect, because this is the PFN of the PML4E strucutre (denoted  as PXE in WinDbg).
