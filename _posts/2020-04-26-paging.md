...
After bit 48, each 8 bits of a virtual address (47-39, 38-30, 29-21, 20-12, 11-0) are actually just indexing various paging structure tables. Note that "indexing" just refers to accessing each paging structure, and reading in/extracting whatever is needed.

Each one of these paging structure tables contains 512 page table entries.
...
The CPU implements a "governor" of sorts, which limits 64-bit addresses to 48-bit addresses. An address in which bits 63-47 are sign extended is known as a Caconical Address.
...
The PML4 indexes the PDPT table and fetches an entry. The PDPT indexes the base of the PDT table and fetches an entry. The PDT table indexes the PT table and fetches a 4 KB physical memory page.
...
We cannot talk about the PML4 table without talking about the CR3 register.
...
Moving on, we can see each strucutre's entries can all be found at their respective virtual addresses as shown above as:
...
A PFN, or page frame number, refers to the next paging structure in the hierarchy.

In the output of `!pte` we see there is an entry called PXE (PML4E). The PML4 table, as we know, will index the PDPT table nextly. This means it will index an entry from the next table, known as a PDPE.

The PFN, as we can see from the output in WinDbg, is `7bbc8` for the PML4 structure. This means this should be the page frame number for the PDPT, as we know a page frame number refers to the next paging structure in the hierarchy. 

We will now use `!vtop` to convert PDPE (PPE in WinDbg) to a physical address to verify that the PML4E entry is indexing the correct paging structure.

<img src="{{ site.url }}{{ site.baseurl }}/images/PAGE_9.png" alt="">

Let's breakdown this command firstly. The `7be59000` value in the above command is the base paging structure in the CR3 register, the PML4 physical address. When using `!vtop`, you use this address to specify the base paging structure. After that, we have the virtual address we want to convert.

As we can see, the PDPT is located at a physical address of `7bbc8000`! This is perfect, because this is the PFN value used by the PML4 structure (denoted as PXE in WinDbg) to index the next paging structure.
...
As we can see, this is true! The physical and virtual memory addresses contain the same values.

One thing we do notice, however, is that the PFN is only `7bbc8` but the actual physical memory address is `7bbc8000`. Why is this?

OH!
---

This is an ideal example to show that a physical page of memory is actually _NOTHING MORE_ than a PFN and an offset to the physical memory page! A PFN, as we can recall, is a reference to the base of the next paging structure. Since we converted the PDPT address (which is a base address to begin with), there was no offset in the physical translation, meaning that the PFN was appended with 0's. If our virtual address would have been `510008`, for instance, our physical address would have been`7bba9008`. This is because the address is at an offset of 0x8 from the base of the PFN!
...
We can clearly see the PFN here, in between the `7A900000` and `867`. But what do these other numbers mean? Additionally, what does `---DA--UWEV` mean? These refer to "control bits", which provision various permissions, features, etc about the memory page. Let's take a look at each of these bits.
...
Again, this was by no means meant to be an exhaustive and comprehensive "tell all" of memory paging. This article pretty much barely scratched the service. However, understanding things like control bits and virtual memory and having that as prerequisite knowledge allows you to understand bypassing mitigations such as NX in kernel pool memory, or more ways of bypassing SMEP. The next post will go into bypassing SMEP and NX in the kernel by way of the prerequisite knowledge laid out here. 
