...
As a point of contention, we have about 980 bytes to work with (as we will see, these 980 bytes will only be used for our first stage ROP/COP payload). Our shellcode will be stored in another location within the process memory.

The Call to `WriteProcessMemory()`
---

What is the goal of this method of bypassing DEP? The goal here is to not to dynamically change permissions of memory to make it executable- but to instead write our shellcode, dynamically, to already executable memory. Additionally, when DEP is enabled, memory is either writable or executable- but not both at the same time. This previous sentiment about writing shellcode, via `WriteProcessMemory()`, to executable memory is a bit contradictory. If memory is executable, then by DEP's rules it _shouldn't_ be able to be written to. `WriteProcessMemory()` overcomes this by temporarily marking memory pages as `RWX` while data is being written to previously executable only pages. After the write succeeds, the memory is then marked again as execute only.
