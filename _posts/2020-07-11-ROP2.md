...
As we know, when DEP is enabled, memory is either writable or executable- but not both at the same time. The previous sentiment about writing shellcode, via `WriteProcessMemory()`, to executable memory is a bit contradictory knowing this. If memory is executable, then by DEP's rules it _shouldn't_ be writable. `WriteProcessMemory()` overcomes this by temporarily marking memory pages as `RWX` while data is being written to a destination- even if that destination doesn't have writable permissions. After the write succeeds, the memory is then marked again as execute only.
