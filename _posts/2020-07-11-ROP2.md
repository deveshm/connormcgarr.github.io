...
After stepping through the `mov ecx, eax` gadget- ECX and EAX are now both set to ESP + 0x24.
...
Recall that EBP is set to ESP + 0x24. This is, as has been mentioned already, to compensate for the `add esp, 0x24` instruction that will preceed the `leave` instruction we have been worried about, where the value of EBP will be placed into ESP. We hope to overcome this by making ESP and EBP equal BEFORE the `leave` instruction occurs.
