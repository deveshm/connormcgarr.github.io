...
To compensate for this, we will manipulate EAX to contain the address at ESP + 0x34.

Why ESP + 0x34 instead of just ESP you ask? This is a "prepatory" procedure (manipulating EAX to contain the address of ESP + 0x34). 
...
In order to do this, the stack will eventually need to be increased by 0x24 bytes to get the value (technically future value) of ESP into ECX, due to the nature of the ROP gadgets available within the process memory. A ROP gadget will inadvertently perform an `add esp, 0x24`, resulting in collateral damage to get what we need accompilshed, accomplished. There will be 4 ROP gadgets, for a total of 0x10 (16 decimal) bytes, that will need to be executed between now and when that `add esp, 0x24` gadget is executed (0x34 - 0x24 = 0x10).

This is reason why we will set EAX to the value of ESP + 0x34 instead of ESP + 0x24, because we will need 0x10 bytes worth of ROP gadgets between then and now. By the time the ROP gadgets _before_ the `add esp, 0x24` instruction are executed, the value in EAX will be ESP + 0x24. If we loaded ESP + 0x24 into EAX now, then by the time we reach the `add esp, 0x24` instruction EAX will contain a value of ESP + 0x14.

Knowing this, and knowing that we would like EAX and ECX to be equal to the current value of ESP _after_ the ESP + 0x24 stack manipulation occurs- we will prepare EAX in advance.
...
`mov esp, ebp` will make EBP and ESP equal to each other. Let's think about how we can leverage this.

We know that currently EAX = ESP + 0x24. We also can recall from earlier that EBP is currently set to 0. If we could place EAX into EBP _BEFORE_ the `leave` instruction executes- it would set ESP to ESP + 0x24 because of the `mov esp, ebp` instruction- which sets ESP to whatever EBP is. Due to the `add esp, 0x24` gadget that occurs before the `leave` instruction- this would actually set ESP = ESP, which is what we want. The goal here is to restore ESP back to our controlled data, which consists of our ROP gadgets.
...
How does adding EAX to EBP place EAX into EBP? Recall that EBP is set to 0 and EAX contains the memory address of ESP + 0x24. That address of ESP + 0x24 will get added to the number 0, which doesn't alter it in any way, and the result of the addition is placed into EBP- essentially "moving" the address into EBP.
...
EBP currently is set to 0 and EAX is set to ESP + 0x24

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM13.png" alt="">

Stepping through the instruction yields the desired result of placing ESP + 0x24 into EBP.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM14.png" alt="">

After EBP is prepared, program execution reaches the next ROP gadget.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM15.png" alt="">

After stepping through the `mov ecx, eax` gadget- ECX and EAX are now both set to ESP + 0x34.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM16.png" alt="">
