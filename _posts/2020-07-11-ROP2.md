...
We know that currently EAX contains our target address. We also can recall from earlier that EBP is currently set to 0. If we could place EAX into EBP _BEFORE_ the `leave` instruction executes- it would set ESP to ESP + 0x24 (at the time of the instruciton executing) because of the `mov esp, ebp` instruction- which sets ESP to whatever EBP is. Due to the `add esp, 0x24` gadget that occurs before the `leave` instruction- this would actually end up ESP to ESP, which is what we want. The goal here is to restore ESP back to our controlled data, which consists of our ROP gadgets.
...
Taking a look on the stack now, we can see there is 0x24 bytes between the address before `0xCCCCCCCC`
