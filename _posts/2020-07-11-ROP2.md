...
Replace the NOPs in the previous PoC script, under the "Begin ROP chain" comment, with the above address. After firing off the updated PoC, we land on our intended ORP gadget.
...
Although EAX is _relatively_ close to ESP- it is still a decent ways away. Currently, EAX (which now contains the same value as EBP) is `0xfec` bytes away from ESP.
...
Why ESP + 0x28 instead of just ESP you ask? This is a "prepatory" procedure (manipulating EAX to contain the address of ESP + 0x28). As we will see later on, we would like to preserve an address around ESP into another "common" register. ECX is a register that is used as a "counter" (although technically it is a general purpose register). This means that ECX generally is a part of some more useful ROP gadgets. 

In order to do this, the stack will be increased by 0x28 bytes to get the value (technically future value) of ESP into ECX, due to the nature of the ROP gadgets available within the process memory. Knowing this, and knowing that we would like EAX and ECX to be equal to the current address at ESP- we will prepare EAX in advance.

> Note that this is by no means a requirement (getting EAX and ECX set to the EXACT value of ESP) when doing ROP. This will just make life easier in the future.
