...
Although the IAT for this application doesn't directly contain a function pointer to `kernel32WriteProcessMemory`- it does contain pointers to other `kernel32.dll` pointers, such as `kernel32!GetStartupInfoA`. We also know that the distance between each function with a DLL DOESN'T CHANGE. This means, the distance between `kernel32!GetStartupInfoA` and `kernel32!WriteProcessMemory` will always remain the same for the current patch level and OS version.
...
The key is now to give some padding between the space on the stack for our future ROP gadgets and our shellcode. To do this, we will provide approximately 0x300 of space on the stack for remaining ROP gadgets. This will allow us to "simulate" the rest of our ROP gadgets and choose a place on the stack that our shellcode will go, and start performing these calculations now. Think of these 0x300 bytes as "ROP gadget placeholders". If perhaps we would need more thatn 0x300 bytes, due to more ROP gadgets needed than anticipated, we would move our shellcode down lower. We will "aim" for 0x300 bytes down the stack, and we will add NOPs to compensate for any of the unused 0x300 bytes (if necessary). The following ROP gadgets can accomplish loading the location of our "shellcode" (future shellcode) into EAX.
...
Utilizing the same gadgets from a previous ROP routine- we can increase ECX by 12 (0xC) decimal bytes, to load the parameter placeholder for `nSize`.
...
It should also be noted, that after each of these ROP gadgets are executed- the AL register will be increased by 0x39 bytes. We will compensate for this in the future. Since AL only backs up the lower 8 bits of the EAX register, this will not have _much_ of an adverse effect on what we are trying to accomplish.
...
Perfect! All that is left now is to is extract our current pointer to `kernel32.dll` and calculate the offset between `kernel32!GetStartupInfoA` and `kernel32!WriteProcessMemory`. After this, we will use the same primitive of dynamically manipulating the `kernel32WriteProcessMemory` parameter placeholder to point to the actual API.
...
Since ECX is reserved for the arbitrary write, we will use EAX to _also_ store the `kernel32!WriteProcessMemroy` parameter placeholder.
...
So currently, as it stands, the stack address of `0x2636920`, which changes when the process restarts, points to `0x1004d1ec`- which then points to the `kernel32.dll` address. This means we have a pointer to a pointer to the pointer we would like to extract. Knowing this, we will dereference `0x2636920` and store the result (which is `0x1004d1ec`) into EAX. Then, utilizing the exact same routine, we will dereference `0x1004d1ec` (which is a pointer to `kernel32!GetStartupInfoA`) and store the result in EAX. We can achieve this with two ROP gadgets.
...
Program execution hits the first gadget, where WinDbg shows us what will be placed in EAX (`0x1004d1ec`).
...
This is great news. We have defeated ASLR on the system itself. What needs to happen now is that we need to find the negative distance between `kernel32!WriteProcessMemory` and `kernel32!GetStartupInfoA`. To do this, we can use WinDbg.
...
Let's use a ROP routine to subtract this value from EAX.
...
crash += struct.pack('<L', 0xfffebeb7)		# Negative distance to kernel32!WriteProcessMemory from kernel32!GetStartupInfoA
...
The negative distance between the two `kernel32.dll` functions is "popped" into EBX.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM77.png" alt="">

Then, the offset is loaded into EDX.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM78.png" alt="">

Program execution then reaches the `sub eax, edx` gadget- which will place the result in EAX.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM79.png" alt="">

We have successfully loaded `kernel32!WriteProcessMemory` into EAX!
