...
Let's say we get the parameter placeholder's locationm (e.g. the virtual memory address, not the `0x11111111` itself) in ECX (which we will). If we were to read the value of ECX, we would be reading the value `0x2826930`. However, if we read the value of `dword ptr [ecx]` instead- we would be reading the actual value of `0x11111111`.

<img src="{{ site.url }}{{ site.baseurl }}/images/WPM33.png" alt="">

The image above shows the value of the address itself. The second part of the image shows what happens when we "dereference", or extract the value a memory address is pointing to. We can leverage this, by using an arbitrary write primtive. When we get the address of the `lpBuffer` parameter into ECX- we then will not overwrite ECX, but rather `dword ptr [ecx]`- which will force the address on the stack (which contains the parameter placeholder) to point to a _DIFFERENT_ address, that we control.
