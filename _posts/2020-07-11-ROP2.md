...
How does adding EAX to EBP place EAX into EBP? Recall that EBP is set to 0 and EAX contains the memory address of ESP + 0x24. That address of ESP + 0x24 will get added to the number 0, which makes it remain the same, and the result of the addition is placed into EBP- essentially "moving" the address into EBP.
