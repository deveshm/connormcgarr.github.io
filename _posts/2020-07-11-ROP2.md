...
We know that currently EAX = ESP + 0x24. We also can recall from earlier that EBP is currently set to 0. If we could place EAX into EBP _BEFORE_ the `leave` instruction executes- it would set ESP to ESP + 0x24 because of the `mov esp, ebp` instruction- which sets ESP to whatever EBP is.

It is a bit of amoutful and "mind bender" of sorts- but this will allow us to resume execution of our ROP gadgets by controlling ESP again. Viewing this step by step in the debugger will help make sense of all of this.
...
How does adding EAX to EBP place EAX into EBP? Recall that EBP is set to 0 and EAX contains a memory address of ESP + 0x28. That address of ESP + 0x28 will get added to the number 0, which makes it remain the same, and the result of the addition is placed into EBP- essentially "moving" the address into EBP.
