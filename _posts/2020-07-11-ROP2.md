...
Why ESP + 0x38 instead of just ESP you ask? This is a "prepatory" procedure (manipulating EAX to contain the address of ESP + 0x38). 
...
In order to do this, the stack will eventually need to be increased by 0x24 bytes to get the value (technically future value) of ESP into ECX, due to the nature of the ROP gadgets available within the process memory. A ROP gadget will inadvertently perform an `add esp, 0x24`, resulting in collateral damage to get what we need accompilshed, accomplished. There will be 4 ROP gadgets (plus an additional DWORD that will be "popped" into a register), for a total of 0x14 (20 decimal) bytes, that will need to be executed between now and when that `add esp, 0x24` gadget is executed (0x34 - 0x24 = 0x10).
...
Program execution then transfers to the `sub eax, ecx` ROP gadget- which will place the difference into the EAX register- yielding our intended result.
