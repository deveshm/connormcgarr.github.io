...
This blog will not address ASLR or 64-bit exploitation- and will be utilizing non-ASLR compiled modules, as well as the x86 `__stdcall` calling convention (technically an "ASLR bypass", but in my opinion only information leak = a _true_ ASLR bypass).
