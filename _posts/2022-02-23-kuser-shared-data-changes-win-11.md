...
This value is then used in a call to `nt!MiReservePtes`, which we can see from two images ago. We know the first argument for this function will go into the RCX register, per the `__fastcall` calling convention. This argument is actually a `nt!_MI_SYSTEM_PTE_TYPE` structure. We know the first argument for this function will go into the RCX register, per the `__fastcall` calling convention. This argument is actually a `nt!_MI_SYSTEM_PTE_TYPE` structure. 
