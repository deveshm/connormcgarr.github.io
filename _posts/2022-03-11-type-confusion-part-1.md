...
We can see that JavaScript just acted _directly_ on the already inlined-values of `1` and `2` and simply just overwrote them with the values provided by `opt()` to update the `o` object. 
