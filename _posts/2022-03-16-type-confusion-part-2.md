...
`DataView` Objects
---
A popular object leveraged for exploitation is a `DataView` object. A [`DataView`](https://github.com/chakra-core/ChakraCore/blob/master/lib/Runtime/Library/DataView.h#L9-L40) object provides users a way to read/write multiple different data types and endianness to and from a raw buffer in memory, which can be created with [`ArrayBuffer`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer). This can include writing or retrieving a 8-byte, 16-byte, 32-byte, or (in some browsers) 64-bytes of raw data from said buffer. More information about `DataView` objects can be found [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/DataView), for the more interested reader.
