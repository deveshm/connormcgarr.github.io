...
We can also infer, based on the above image, that this isn't a fast call, but a _rep_ (repeat) call. Repeat hypercalls are broken up into a "series" of hypercalls because hypercalls only have a 50 microsecond interval to finish before other components (interrupts for instance) need to be services. Repeated hypercalls will eventually be finished when the thread executing the hypercall resumes.
